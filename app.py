...
def generate_html_report(df_comp_fmt, figures_dict):
    ...
    <head>
        ...
        <title>Relatório Executivo de Risco</title>
        ...
        <style>
            ...
            .header-banner {{ background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: white; padding: 40px; border-radius: 15px; margin-bottom: 30px; text-align: center; }}
            .card {{ border: none; border-radius: 15px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); margin-bottom: 30px; }}
            .card-header {{ background-color: white; border-bottom: 1px solid #eee; font-weight: bold; font-size: 1.25rem; padding: 20px; border-radius: 15px 15px 0 0 !important; }}
            .table {{ font-size: 0.95rem; }}
            .metric-val {{ font-weight: bold; color: #1e3c72; }}
    ...
    <body>
        <div class="container">
            <div class="header-banner">
                <h1 class="display-5 fw-bold">Executive Risk Report</h1>
                <p class="lead">Portfolio Analysis & Risk Management System</p>
                <p class="mb-0">Gerado em: {now_str}</p>
            </div>

            <div class="card">
                <div class="card-header">📊 Métricas de Performance Consolidada</div>
                <div class="card-body">
                    <div class="table-responsive">
                        {df_comp_fmt.to_html(classes='table table-hover', index=False, border=0)}
                    </div>
                </div>
            </div>
    """
    ...
    html_content += """
            <footer>
                <p>&copy; 2024 Portfolio Risk Management System | Desenvolvido para análise quantitativa profissional.</p>
            </footer>
        </div>
    </body>
    </html>
...
with col_kpi:
    ...
    # Criar DF formatado para exibição e relatório
    ...
    st.dataframe(
        df_comp_fmt.set_index("Metric").style.applymap(highlight_kpi, subset=(["Retorno do Período"], slice(None))), 
...
with tab4:
    fig_corr = px.imshow(assets_ret.corr(), text_auto=".2f", aspect="auto", color_continuous_scale="RdYlGn", zmin=-1, zmax=1)
    st.plotly_chart(fig_corr, use_container_width=True)

with tab5:
    cum_orig, cum_sim, cum_bench = (1 + ret_orig).cumprod(), (1 + ret_sim).cumprod(), (1 + bench_ret).cumprod()
    fig_cum = go.Figure()
    fig_cum.add_trace(go.Scatter(x=cum_orig.index, y=cum_orig, name="Current (Fixed)"))
    fig_cum.add_trace(go.Scatter(x=cum_sim.index, y=cum_sim, name=f"Simulated ({rebal_freq_sim})"))
    fig_cum.add_trace(go.Scatter(x=cum_bench.index, y=cum_bench, name="Benchmark"))
...
# ==============================================================================
# 6. RELATÓRIOS (SIDEBAR)
# ==============================================================================
st.sidebar.markdown("---")
st.sidebar.subheader("📄 Reporting")

# Captura das figuras geradas para o relatório
figs_report = {
    "Risco vs Retorno": fig1,
    "Qualidade da Volatilidade": fig2,
    "Captura de Mercado": fig3,
    "Matriz de Correlação": fig_corr,
    "Performance Acumulada": fig_cum
}

if st.sidebar.button("Generate Executive Report (HTML)", use_container_width=True, type="primary"):
    html_data = generate_html_report(df_comp_fmt, figs_report)
